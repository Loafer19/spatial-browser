...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (style.size_radius.w > 0.5) {
        ...
        color = textureSample(tex0, samp0, input.tex);
    ...
    // Top-edge load bar (Chrome-style). Height follows theme focus border.
    // Progress from CEF; if still ~0 while active, a sliding chunk shows.
    if (style.load.y > 0.5 && outside_alpha > 0.01) {
        let bar_h = max(style.load.z, 2.0);
        let py = input.tex.y * size.y;
        if (py < bar_h) {
            var filled = 0.0;
            if (style.load.x < 0.02) {
                let t = fract(frame.time.x * 0.7);
                let start = t * 1.3 - 0.3;
                filled = select(0.0, 1.0, input.tex.x >= start && input.tex.x <= (start + 0.3));
            } else {
                filled = select(0.0, 1.0, input.tex.x <= style.load.x);
            }
            let track = border_color * 0.35;
            let fill = style.focus_border_color.rgb;
            color = vec4<f32>(mix(track, fill, filled), 1.0);
            color.a = color.a * outside_alpha;
        }
    }
